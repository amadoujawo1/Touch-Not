...
@login_required
def change_password():
    # Redirect to appropriate dashboard if not first login
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # Force logout if role is not recognized
            logout_user()
            flash('Invalid role detected. Please contact administrator.', 'warning')
            return redirect(url_for('auth.login'))
